...
func _generate_map_collision():
	...
	if map == null:
		return
	# Collect all visible mesh instances
	...
	# Add collision as child of each mesh (inherits transform)
	for m in meshes:
		...
		body.add_child(col)
	print("Base01: Generated collision for %d meshes" % meshes.size())
...
func spawn_weapon_reward():
	...
	if player:
		...
		pickup.global_position = Vector3(-22, 16, -2)
	add_child(pickup)
